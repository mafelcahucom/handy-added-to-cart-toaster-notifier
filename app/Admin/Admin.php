...
namespace HATFW\Admin;

use HATFW\Inc\Traits\Singleton;
use HATFW\Inc\Traits\Instantiator;
use HATFW\Admin\Inc\Helper;
use HATFW\Admin\Modules\Setting\Setting;
use HATFW\Admin\Modules\ImporterExporter\ImporterExporter;
...
final class Admin {
    ...
    public function register_menu_styles() {
        ...
        $asset['src'] = Helper::get_public_src( 'styles/hatfw-home.css' );
        wp_register_style( 'hatfw-home', $asset['src'], $asset['dependencies'], $asset['version'], 'all' );
        wp_enqueue_style( 'hatfw-home' );
    }
    ...
    public function register_submenu_scripts() {
        ...
        $asset        = include HATFW_PLUGIN_PATH . 'public/admin/scripts/hatfw-admin.asset.php';
        $asset['src'] = Helper::get_public_src( 'scripts/hatfw-admin.js' );
        wp_register_script( 'hatfw-admin', $asset['src'], $asset['dependencies'], $asset['version'], true );
        wp_enqueue_script( 'hatfw-admin' );

        wp_localize_script( 'hatfw-admin', 'hatfwLocal', array(
            'api'    => 'HNJOELMAFUCOHACM',
            'url'    => admin_url( 'admin-ajax.php' ),
            'module' => array(
                'setting' => array(
                    'nonce' => array(
                        'saveSettings' => wp_create_nonce( 'hatfw_save_settings' ),
                    ),
                ),
                'importerExporter' => array(
                    'nonce' => array(
                        'exportSettings' => wp_create_nonce( 'hatfw_export_settings' ),
                        'importSettings' => wp_create_nonce( 'hatfw_import_settings' ),
                    ),
                ),
            ),
        ));
