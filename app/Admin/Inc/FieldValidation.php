...
final class FieldValidation {
     ...
     *     'current_value' => (string)  Contains the thee current value from _hatfw_main_settings.
     * ]
     * @return array
     */
        ...
        if ( ! empty( $args['value'] ) ) {
            if ( strlen( $args['value'] ) <= $args['max'] ) {
                ...
                $output['error'] = sprintf(
                    __( 'Total characters must not exceed in %s.', HATFW_PLUGIN_DOMAIN ),
                    $args['max']
                );
            }
        }
    ...
    public static function validate_number_field( $args = [] ) {
        ...
        if ( $args['value'] !== '' && $args['value'] !== null ) {
            ...
            if ( is_numeric( $number ) ) {
                ...
                $output['error'] = __( 'Invalid value. Please enter the appropriate value.', HATFW_PLUGIN_DOMAIN );
            }
        }
    ...
    public static function validate_switch_field( $args = [] ) {
        $output = [
            'success' => false,
            'value'   => $args['current_value'],
            'error'   => __( 'Invalid value. Please enter the appropriate value.', HATFW_PLUGIN_DOMAIN )
        ];
    ...
    public static function validate_select_field( $args = [] ) {
        $output = [
            'success' => false,
            'value'   => $args['current_value'],
            'error'   => __( 'This field is required.', HATFW_PLUGIN_DOMAIN )
        ];
    ...
    public static function validate_icon_picker_field( $args = [] ) {
        ...
        if ( ! empty( $args['value'] ) ) {
            if ( in_array( $args['value'], $args['icons'] ) ) {
                ...
                $output['error'] = __( 'Invalid icon. Please select the appropriate icon.', HATFW_PLUGIN_DOMAIN );
            }
        }
    ...
    public static function validate_image_picker_field( $args = [] ) {
        ...
        if ( ! empty( $args['value'] ) ) {
            if ( in_array( $args['value'], $args['choices'] ) ) {
                ...
                $output['error'] = __( 'Invalid image. Please select the appropriate image.', HATFW_PLUGIN_DOMAIN );
            }
        }
    ...
    public static function validate_loader_picker_field( $args = [] ) {
        ...
        if ( ! empty( $args['value'] ) ) {
            if ( in_array( $args['value'], $args['choices'] ) ) {
                ...
                $output['error'] = __( 'Invalid loader. Please select the appropriate loader.', HATFW_PLUGIN_DOMAIN );
            }
        }
    ...
    public static function validate_color_picker_field( $args = [] ) {
        ...
        if ( ! empty( $args['value'] ) ) {
            if ( preg_match( '/^rgba\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3}),\s*(\d*(?:\.\d+)?)\)$/', $args['value'] ) == true ) {
                ...
                $output['error'] = __( 'Invalid color format. Please enter the appropriate color format.', HATFW_PLUGIN_DOMAIN );
            }
        }
